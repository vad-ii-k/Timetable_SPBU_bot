...
class TimetableIMG:
    ...
    final_img_width, final_img_height = (1500, 2600)
    font_h1 = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Medium.ttf", size=60)
    font_h2 = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Medium.ttf", size=45)
    font_h3 = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Medium.ttf", size=35)
    font_reqular = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Regular.ttf", size=30)
    font_bold = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Bold.ttf", size=30)
    font_italic = ImageFont.truetype(font="utils/image_converter/FiraSansFonts/FiraSans-Italic.ttf", size=30)
    ...
    def insert_timetable(self, timetable: str):
        ...
        for i in range(1, len(text), 5):
            if text[i + 1].find('\n') == -1:
                ...
                draw.multiline_text(xy=(x, y), align="left", text="  {}".format(text[i + 1]), font=TimetableIMG.font_bold, fill="black")
            ...
            y += TimetableIMG.font_italic.size + skip
            draw.line(xy=[(x + 4, self.y), (x + 4, y - skip//2)], fill="red", width=5)
            draw.multiline_text(xy=(10, self.y),
