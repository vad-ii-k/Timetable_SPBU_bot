...
class TimetableIMG:
    ...
    _font_h1 = ImageFont.truetype(font="data/converter/fonts/FiraSans-Medium.ttf", size=100)
    _font_h2 = ImageFont.truetype(font="data/converter/fonts/FiraSans-Medium.ttf", size=70)
    _font_h3 = ImageFont.truetype(font="data/converter/fonts/FiraSans-Medium.ttf", size=50)
    _font_reqular = ImageFont.truetype(font="data/converter/fonts/FiraSans-Regular.ttf", size=40)
    _font_bold = ImageFont.truetype(font="data/converter/fonts/FiraSans-Bold.ttf", size=45)
    _font_italic = ImageFont.truetype(font="data/converter/fonts/FiraSans-Italic.ttf", size=40)
    ...
    def _create_foundation() -> Image:
        ...
        with Image.open("data/converter/pictures_for_use/spbu.png") as image_spbu:
            image.paste(
                im=image_spbu,
                box=(image.width - image_spbu.width - 30, 0),
                mask=image_spbu,
            )
...
self, xy: tuple, text: str, font: ImageFont, event_cancelled: bool = False
        ...
        draw = ImageDraw.Draw(image)
        x, y = xy
        lines = textwrap.wrap(text, width=int(self._final_img_width * 0.88 // font.size))
        for line in lines:
    ...
    def _insert_events(
            self, draw: ImageDraw, skip: int, indent: int, x: int, y: int, events: List[StudyEvent]
    ) -> None:
        for i, event in enumerate(events):
            if i == 0 or is_basic_events_info_identical(events[i - 1], events[i]):
                if (
                        i == 0
                        or events[i - 1].start_time != event.start_time
                        or events[i - 1].end_time != event.end_time
                ):
                    event_time = f"{event.start_time:%H:%M}\n{event.end_time:%H:%M}"
                    draw.text(
                        xy=(x - indent, y),
                        text=event_time,
                        font=TimetableIMG._font_reqular,
                        fill="grey" if event.is_canceled else "black",
                    )
                y = self._draw_text(
                    xy=(x, y),
                    text=event.subject_name,
                    font=TimetableIMG._font_bold,
                    event_cancelled=event.is_canceled,
                )
                y = self._draw_text(
                    xy=(x, y),
                    text=event.subject_format,
                    font=TimetableIMG._font_italic,
                    event_cancelled=event.is_canceled,
                )
            y_for_subject_line = y
            y = self._draw_text(
                xy=(x + 25, y),
                text=event.contingent,
                font=TimetableIMG._font_italic,
                event_cancelled=event.is_canceled,
            )
            y = self._draw_text(
            ...
            draw.line(
                xy=[(x + 25, y_for_subject_line + skip // 2), (x + 25, y - skip // 2)],
                fill="grey" if event.is_canceled else "#0088cc",
                width=3,
            )
    ...
    def crop_image(self) -> None:
        image = self._current_image
        image = image.crop(
            box=(0, 0, self._final_img_width, min(self._y_max + 15, self._final_img_height))
        )
