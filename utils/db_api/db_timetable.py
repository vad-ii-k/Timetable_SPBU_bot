...
async def add_timetable_to_db(events: List[Dict[str, str | bool]], tt_id: int,
                              user_type: str, full_name: str = None) -> None:
    for event in events:
        ...
        if user_type == 'student':
            ...
            await loader.db.add_new_group_study_event(int(tt_id), db_subject.subject_id, event_date,
                                                      start, end, educator, is_cancelled)
        else:
            groups: str = event.get("ContingentUnitName")
            await loader.db.add_new_teacher_study_event(int(tt_id), full_name, db_subject.subject_id, event_date,
                                                        start, end, groups, is_cancelled)


async def get_timetable_day_from_db(db_id: int, current_date: date, user_type: str) -> List[TimetableOneDay]:
    timetable_info = []
    if user_type == 'student':
...
async def get_timetable_week_from_db(db_id: int, monday: date, sunday: date,
                                     user_type: str) -> List[TimetableOneDay]:
    timetable_info = []
    if user_type == 'student':
    ...
    while i < len(study_events):
        ...
        while i < len(study_events) and day == study_events[i].date:
            event = StudyEvent(
                ...
                contingent=study_events[i].educator if user_type == 'student' else study_events[i].groups,
