...
async def get_timetable(tt_id: id, is_picture: bool, user_type: str,
    ...
    if week_counter is not None:
        ...
        if is_picture:
            timetable_text = await get_image_timetable_week(tt_id, tt_name, monday, sunday, timetable_db, user_type)
        else:
            timetable_text = await get_text_timetable_week(tt_id, tt_name, monday, sunday, timetable_db, user_type)
    else:
        ...
        if is_picture:
            timetable_text = await get_image_timetable_day(tt_id, tt_name, current_date, timetable_db, user_type)
        else:
            timetable_text = await get_text_timetable_day(tt_id, tt_name, current_date, timetable_db, user_type)
    return timetable_text


async def get_text_timetable_day(tt_id: int, tt_name: str, current_date: date,
                                 timetable_db: List[TimetableOneDay], user_type: str) -> str:
    timetable = ''
    ...
    if len(timetable_db[0].events) > 0:
        day_timetable = await timetable_parser_day(day=timetable_db[0].date, events=timetable_db[0].events,
                                                   user_type='teacher')
...
async def get_image_timetable_day(tt_id: int, tt_name: str, current_date: date,
                                  timetable_db: List[TimetableOneDay], user_type: str) -> str:
    timetable = ''
    if user_type == 'teacher':
        timetable = await teacher_timetable_day_header(tt_id, current_date, tt_name)
    ...
    schedule_pic.create_image_title(title=tt_name, date=format_date(current_date, 'EEEE, d MMMM', locale='ru_RU'))
...
async def get_text_timetable_week(tt_id: int, tt_name: str, monday: date, sunday: date,
                                  timetable_db: List[TimetableOneDay], user_type: str) -> str:
    timetable = ''
    if user_type == 'teacher':
    ...
    if len(timetable_db) > 0:
        for day in timetable_db:
            day_timetable = await timetable_parser_day(day=day.date, events=day.events, user_type='teacher')
            if len(timetable) + len(day_timetable) < 4096:
                timetable += day_timetable
            else:
                timetable += "\n\n📛 Сообщение слишком длинное..."
...
async def get_image_timetable_week(tt_id: int, tt_name: str, monday: date, sunday: date,
                                   timetable_db: List[TimetableOneDay], user_type: str) -> str:
    timetable = ''
    if user_type == 'teacher':
        timetable = await teacher_timetable_week_header(tt_id, monday, sunday, tt_name)
