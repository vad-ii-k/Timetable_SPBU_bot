...
async def get_teacher_timetable(tt_id: id, is_picture: bool, week_counter: int = None, day_counter: int = None) -> str:
    ...
    if week_counter is not None:
        ...
        if is_picture:
            timetable_text = await get_image_teacher_timetable_week(tt_id, teacher_spbu_db.full_name, monday, sunday,
                                                                    timetable_db)
        else:
            timetable_text = await get_text_teacher_timetable_week(tt_id, teacher_spbu_db.full_name, monday, sunday,
                                                                   timetable_db)
    else:
        ...
        if is_picture:
            timetable_text = await get_image_teacher_timetable_day(tt_id, teacher_spbu_db.full_name, current_date,
                                                                   timetable_db)
        else:
            timetable_text = await get_text_teacher_timetable_day(tt_id, teacher_spbu_db.full_name, current_date,
                                                                  timetable_db)
...
async def get_image_teacher_timetable_week(group_id: int, group_name: str, monday: date, sunday: date,
                                           timetable_db) -> str:
