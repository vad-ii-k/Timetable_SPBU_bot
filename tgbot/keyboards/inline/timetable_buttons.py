...
async def create_timetable_keyboard(is_picture: bool, tt_id: str, day_counter: int = 0) -> InlineKeyboardMarkup:
    ...
    prev_day_button = InlineKeyboardButton(
        text=f"⬅ {prev_day_date:%d.%m}", callback_data=timetable_callback.new(button="1-1", tt_id=tt_id),
    )
    today_button = InlineKeyboardButton(
        text="Сегодня", callback_data=timetable_callback.new(button="1-2", tt_id=tt_id)
    )
    next_day_button = InlineKeyboardButton(
        text=f"{next_day_date:%d.%m} ➡️", callback_data=timetable_callback.new(button="1-3", tt_id=tt_id),
    )
    if day_counter > -7:
    ...
    this_week_button = InlineKeyboardButton(
        text="⏹ Эта неделя", callback_data=timetable_callback.new(button="2-1", tt_id=tt_id)
    )
    next_week_button = InlineKeyboardButton(
        text="След. неделя ⏩", callback_data=timetable_callback.new(button="2-2", tt_id=tt_id)
    )
    ...
    schedule_view = InlineKeyboardButton(
        text=text, callback_data=timetable_callback.new(button="3-1", tt_id=tt_id)
    )
